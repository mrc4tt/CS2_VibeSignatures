...
- Add: `gamesymbol_candidate.py`
  - candidate build、compare、session state 和 publish。
...
- Modify: `gamesymbol_pr_validation.py`
  - analysis 后生成 actual candidate。
  - 比较 candidate 与 head snapshot，不再以 actual bin 作为 downstream input。
...
- Modify: `tests/test_cpp_tests_util.py`
  - snapshot glob、merged module、vtable/record parity。
- Modify: `tests/test_run_cpp_tests.py`
  - CLI source selection 和 candidate-only integration。
