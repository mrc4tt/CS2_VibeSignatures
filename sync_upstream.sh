...
PROTECTED_PATHS=(
    ...
    # in .gitmodules and the gitlink itself are plain content to -X theirs, so
    # without this every sync would silently hand the vtable headers back --
    # and cpp_tests derives vfunc_index from exactly those headers.
    .gitmodules
    hl2sdk_cs2
    gamedata
    gamesymbols
    pages
    # Generators this fork disabled (MODULE_ENABLED = False). Belt and braces
    # next to .gitattributes merge=ours: -X theirs has priority over a lot, and
    # a silent flip back to True would put five unwanted plugins back into every
    # gamedata run without anything failing.
...
for path in "${PROTECTED_PATHS[@]}"; do
    ...
    # A gitlink is one entry, not a tree, so the clear-then-restore below would
    # delete the submodule's working tree and leave an empty directory behind.
    # Restore the entry and re-check-out the submodule instead.
