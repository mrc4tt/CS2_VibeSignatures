#!/usr/bin/env python3
"""
bot-improver Gamedata Update Module (weaponpaints pattern)

Updates bot-improver.json for the WeaponPaints CounterStrikeSharp plugin.
Same gamedata.json schema as CounterStrikeSharp, so reuses the CSS converters.

Key naming follows the plugin repo's gamedata/weaponpaints.json (e.g.
"CAttributeList::SetOrAddAttributeValueByName"). :: is only normalized when
looking up the analysis symbol; JSON keys are written back untouched so
::-style key names survive every regeneration.
"""
...
# Seed source: https://raw.githubusercontent.com/ed0ard/CS2-Bot-Improver/main/addons/BotController/gamedata.json
# Refresh: curl -o gamedata/bot-improver.json <url>
# NB: BotProfile_* entries her er structmembers (category: structmember i configs).

# Module metadata
MODULE_NAME = "bot-improver"
MODULE_ENABLED = True

# Relative path to the gamedata file within the module output directory
GAMEDATA_PATH = "gamedata/bot-improver.json"
OUTPUT_PATHS = (GAMEDATA_PATH,)

# Seed new output roots with the current file from the plugin repo. The static
# copy is refreshed from upstream (merge semantics: only signatures/offsets of
# symbols with analysis YAML data are rewritten):
#   https://git.miksen.me/mikkel/weaponpaints/raw/branch/main/gamedata/weaponpaints.json
