---
name: find-CBasePlayerController_FakeClientFlags
description: |
  Locate CBasePlayerController::FakeClientFlags in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the local gamedata entry "CBasePlayerController::FakeClientFlags" (symbol CBasePlayerController_FakeClientFlags). This is a VIRTUAL function - resolve the CBasePlayerController::FakeClientFlags vtable via RTTI and identify the
...
commonly differ between platforms - never assume identical indices.
  Trigger: CBasePlayerController_FakeClientFlags, CBasePlayerController::FakeClientFlags
disable-model-invocation: true
---

# Find CBasePlayerController_FakeClientFlags

Target: `CBasePlayerController::FakeClientFlags` (vfunc) in the CS2 server module.
...
This is a VIRTUAL function - resolve the CBasePlayerController::FakeClientFlags vtable via RTTI and identify the
slot, then emit the slot. Verify by xrefs from expected call sites. vtable slots
commonly differ between platforms - never assume identical indices.
