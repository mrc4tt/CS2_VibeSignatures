---
name: find-CCSPlayerPawn_SetModelFromLoadout
description: |
  Locate CCSPlayerPawn::SetModelFromLoadout in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the WeaponPaints gamedata entry "CCSPlayerPawn::SetModelFromLoadout" (symbol CCSPlayerPawn_SetModelFromLoadout). Shortlist alongside SetModelFromClass (same file/call region — sibling paths in pawn model selection). This variant resolves the model from the player's loadout/inventory items (agent skin selection), referencing inventory services and item definition indexes before calling SetModel.
  Trigger: CCSPlayerPawn_SetModelFromLoadout, CCSPlayerPawn::SetModelFromLoadout
disable-model-invocation: true
---

# Find CCSPlayerPawn_SetModelFromLoadout

Target: `CCSPlayerPawn::SetModelFromLoadout` (func) in the CS2 server module, both `server.dll` (windows) and `libserver.so` (linux).
...
Shortlist alongside SetModelFromClass (same file/call region — sibling paths in pawn model selection). This variant resolves the model from the player's loadout/inventory items (agent skin selection), referencing inventory services and item definition indexes before calling SetModel.

## Output schema (STRICT)

Write the YAML file `CCSPlayerPawn_SetModelFromLoadout.{platform}.yaml` with EXACTLY these fields — no more, no fewer.
