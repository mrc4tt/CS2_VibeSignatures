---
name: find-CServerSideClient_m_Server
description: |
  Locate CServerSideClient::m_Server in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the local gamedata entry "CServerSideClient::m_Server" (symbol CServerSideClient_m_Server). This is a STRUCT MEMBER OFFSET (schema netvar), not a function. Resolve the
...
natural alignment. Cross-check with functions that dereference the member.
  Trigger: CServerSideClient_m_Server, CServerSideClient::m_Server
disable-model-invocation: true
---

# Find CServerSideClient_m_Server

Target: `CServerSideClient::m_Server` (structmember) in the CS2 server module.
