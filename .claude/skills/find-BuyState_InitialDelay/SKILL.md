...
description: |
  Locate BuyState::InitialDelay in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the local gamedata entry "BuyState::InitialDelay" (symbol BuyState_InitialDelay). This is a VIRTUAL function - resolve the BuyState::InitialDelay vtable via RTTI and identify the
...
Target: `BuyState::InitialDelay` (vfunc) in the CS2 server module.
...
This is a VIRTUAL function - resolve the BuyState::InitialDelay vtable via RTTI and identify the
slot, then emit the slot. Verify by xrefs from expected call sites. vtable slots
commonly differ between platforms - never assume identical indices.
