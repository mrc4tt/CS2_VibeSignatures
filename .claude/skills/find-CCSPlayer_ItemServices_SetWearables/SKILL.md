---
name: find-CCSPlayer_ItemServices_SetWearables
description: |
  Locate CCSPlayer_ItemServices::SetWearables in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the WeaponPaints gamedata entry "CCSPlayer_ItemServices::SetWearables" (symbol CCSPlayer_ItemServices_SetWearables). VIRTUAL function, but WeaponPaints consumes it as a byte signature. Resolve the CCSPlayer_ItemServices vtable via RTTI to locate the slot, then generate a function-head byte signature of the resolved function. The body copies wearable item handles into the pawn's wearable array (m_Item wearables netvar array) and marks state changed.
  Trigger: CCSPlayer_ItemServices_SetWearables, CCSPlayer_ItemServices::SetWearables
disable-model-invocation: true
---

# Find CCSPlayer_ItemServices_SetWearables

Target: `CCSPlayer_ItemServices::SetWearables` (func) in the CS2 server module, both `server.dll` (windows) and `libserver.so` (linux).
...
VIRTUAL function, but WeaponPaints consumes it as a byte signature. Resolve the CCSPlayer_ItemServices vtable via RTTI to locate the slot, then generate a function-head byte signature of the resolved function. The body copies wearable item handles into the pawn's wearable array (m_Item wearables netvar array) and marks state changed.
...
Write the YAML file `CCSPlayer_ItemServices_SetWearables.{platform}.yaml` with EXACTLY these fields — no more, no fewer.
