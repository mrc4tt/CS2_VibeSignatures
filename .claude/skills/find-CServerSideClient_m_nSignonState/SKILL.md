---
name: find-CServerSideClient_m_nSignonState
description: |
  Locate CServerSideClient::m_nSignonState in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the local gamedata entry "CServerSideClient::m_nSignonState" (symbol CServerSideClient_m_nSignonState). This is a STRUCT MEMBER OFFSET (schema netvar), not a function. Resolve the
...
natural alignment. Cross-check with functions that dereference the member.
  Trigger: CServerSideClient_m_nSignonState, CServerSideClient::m_nSignonState
disable-model-invocation: true
---

# Find CServerSideClient_m_nSignonState

Target: `CServerSideClient::m_nSignonState` (structmember) in the CS2 server module.
