---
name: find-CMoveData_AbsOrigin
description: |
  Locate CMoveData::AbsOrigin in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the local gamedata entry "CMoveData::AbsOrigin" (symbol CMoveData_AbsOrigin). This is a VIRTUAL function - resolve the CMoveData::AbsOrigin vtable via RTTI and identify the
...
commonly differ between platforms - never assume identical indices.
  Trigger: CMoveData_AbsOrigin, CMoveData::AbsOrigin
disable-model-invocation: true
---

# Find CMoveData_AbsOrigin

Target: `CMoveData::AbsOrigin` (vfunc) in the CS2 server module.
...
This is a VIRTUAL function - resolve the CMoveData::AbsOrigin vtable via RTTI and identify the
slot, then emit the slot. Verify by xrefs from expected call sites. vtable slots
commonly differ between platforms - never assume identical indices.
