...
description: |
  ...
  signature or offset for the local gamedata entry "CCSPlayer_MovementServices::Pawn" (symbol CCSPlayer_MovementServices_Pawn). This is a VIRTUAL function - resolve the CCSPlayer_MovementServices::Pawn vtable via RTTI and identify the
...
Target: `CCSPlayer_MovementServices::Pawn` (vfunc) in the CS2 server module.
...
This is a VIRTUAL function - resolve the CCSPlayer_MovementServices::Pawn vtable via RTTI and identify the
slot, then emit the slot. Verify by xrefs from expected call sites. vtable slots
commonly differ between platforms - never assume identical indices.
