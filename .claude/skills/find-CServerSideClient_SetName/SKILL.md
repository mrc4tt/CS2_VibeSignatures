---
name: find-CServerSideClient_SetName
description: |
  Locate CServerSideClient::SetName in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the local gamedata entry "CServerSideClient::SetName" (symbol CServerSideClient_SetName). This is a VIRTUAL function - resolve the CServerSideClient::SetName vtable via RTTI and identify the
...
commonly differ between platforms - never assume identical indices.
  Trigger: CServerSideClient_SetName, CServerSideClient::SetName
disable-model-invocation: true
---

# Find CServerSideClient_SetName

Target: `CServerSideClient::SetName` (vfunc) in the CS2 server module.
...
This is a VIRTUAL function - resolve the CServerSideClient::SetName vtable via RTTI and identify the
slot, then emit the slot. Verify by xrefs from expected call sites. vtable slots
commonly differ between platforms - never assume identical indices.
