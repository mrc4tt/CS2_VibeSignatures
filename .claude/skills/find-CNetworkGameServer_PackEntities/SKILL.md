---
name: find-CNetworkGameServer_PackEntities
description: |
  Locate CNetworkGameServer::PackEntities in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the local gamedata entry "CNetworkGameServer::PackEntities" (symbol CNetworkGameServer_PackEntities). This is a non-virtual function - emit a byte signature (func_sig), not an offset.
...
of related symbols; verify by decompilation before committing to a candidate.
  Trigger: CNetworkGameServer_PackEntities, CNetworkGameServer::PackEntities
disable-model-invocation: true
---

# Find CNetworkGameServer_PackEntities

Target: `CNetworkGameServer::PackEntities` (func) in the CS2 server module.
