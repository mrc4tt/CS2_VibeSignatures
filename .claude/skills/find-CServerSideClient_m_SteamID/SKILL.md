---
name: find-CServerSideClient_m_SteamID
description: |
  Locate CServerSideClient::m_SteamID in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the local gamedata entry "CServerSideClient::m_SteamID" (symbol CServerSideClient_m_SteamID). This is a STRUCT MEMBER OFFSET (schema netvar), not a function. Resolve the
...
natural alignment. Cross-check with functions that dereference the member.
  Trigger: CServerSideClient_m_SteamID, CServerSideClient::m_SteamID
disable-model-invocation: true
---

# Find CServerSideClient_m_SteamID

Target: `CServerSideClient::m_SteamID` (structmember) in the CS2 server module.
