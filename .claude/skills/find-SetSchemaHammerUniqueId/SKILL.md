---
name: find-SetSchemaHammerUniqueId
description: |
  Locate SetSchemaHammerUniqueId in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the local gamedata entry "SetSchemaHammerUniqueId" (symbol SetSchemaHammerUniqueId). This is a STRUCT MEMBER OFFSET (schema netvar), not a function. Resolve the
  None class layout via the schema/network system; the member must satisfy
  ...
  Trigger: SetSchemaHammerUniqueId, SetSchemaHammerUniqueId
disable-model-invocation: true
---

# Find SetSchemaHammerUniqueId

Target: `SetSchemaHammerUniqueId` (patch) in the CS2 server module.
