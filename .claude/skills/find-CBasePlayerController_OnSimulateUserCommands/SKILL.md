---
name: find-CBasePlayerController_OnSimulateUserCommands
description: |
  Locate CBasePlayerController::OnSimulateUserCommands in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the local gamedata entry "CBasePlayerController::OnSimulateUserCommands" (symbol CBasePlayerController_OnSimulateUserCommands). This is a non-virtual function - emit a byte signature (func_sig), not an offset.
...
of related symbols; verify by decompilation before committing to a candidate.
  Trigger: CBasePlayerController_OnSimulateUserCommands, CBasePlayerController::OnSimulateUserCommands
disable-model-invocation: true
---

# Find CBasePlayerController_OnSimulateUserCommands

Target: `CBasePlayerController::OnSimulateUserCommands` (func) in the CS2 server module.
