---
name: find-CServerSideClient_m_UserID
description: |
  Locate CServerSideClient::m_UserID in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the local gamedata entry "CServerSideClient::m_UserID" (symbol CServerSideClient_m_UserID). This is a STRUCT MEMBER OFFSET (schema netvar), not a function. Resolve the
...
natural alignment. Cross-check with functions that dereference the member.
  Trigger: CServerSideClient_m_UserID, CServerSideClient::m_UserID
disable-model-invocation: true
---

# Find CServerSideClient_m_UserID

Target: `CServerSideClient::m_UserID` (structmember) in the CS2 server module.
