---
name: find-BotProfile_ReactionTime
description: |
  Locate BotProfile::ReactionTime in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the local gamedata entry "BotProfile::ReactionTime" (symbol BotProfile_ReactionTime). This is a VIRTUAL function - resolve the BotProfile::ReactionTime vtable via RTTI and identify the
...
commonly differ between platforms - never assume identical indices.
  Trigger: BotProfile_ReactionTime, BotProfile::ReactionTime
disable-model-invocation: true
---

# Find BotProfile_ReactionTime

Target: `BotProfile::ReactionTime` (vfunc) in the CS2 server module.
...
This is a VIRTUAL function - resolve the BotProfile::ReactionTime vtable via RTTI and identify the
slot, then emit the slot. Verify by xrefs from expected call sites. vtable slots
commonly differ between platforms - never assume identical indices.
