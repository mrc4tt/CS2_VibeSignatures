---
name: find-BotProfile_WeaponPref
description: |
  Locate BotProfile::WeaponPref in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the local gamedata entry "BotProfile::WeaponPref" (symbol BotProfile_WeaponPref). This is a VIRTUAL function - resolve the BotProfile::WeaponPref vtable via RTTI and identify the
  ...
  Trigger: BotProfile_WeaponPref, BotProfile::WeaponPref
disable-model-invocation: true
---

# Find BotProfile_WeaponPref

Target: `BotProfile::WeaponPref` (vfunc) in the CS2 server module.
...
This is a VIRTUAL function - resolve the BotProfile::WeaponPref vtable via RTTI and identify the
slot, then emit the slot. Verify by xrefs from expected call sites. vtable slots
commonly differ between platforms - never assume identical indices.
