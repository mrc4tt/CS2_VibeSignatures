---
name: find-BotProfile_LookAngleMaxAccelAttacking
description: |
  Locate BotProfile::LookAngleMaxAccelAttacking in CS2 server.dll / libserver.so via IDA Pro MCP and emit a fresh, minimal-unique
  signature or offset for the local gamedata entry "BotProfile::LookAngleMaxAccelAttacking" (symbol BotProfile_LookAngleMaxAccelAttacking). This is a VIRTUAL function - resolve the BotProfile::LookAngleMaxAccelAttacking vtable via RTTI and identify the
  ...
  Trigger: BotProfile_LookAngleMaxAccelAttacking, BotProfile::LookAngleMaxAccelAttacking
disable-model-invocation: true
---

# Find BotProfile_LookAngleMaxAccelAttacking

Target: `BotProfile::LookAngleMaxAccelAttacking` (vfunc) in the CS2 server module.
...
This is a VIRTUAL function - resolve the BotProfile::LookAngleMaxAccelAttacking vtable via RTTI and identify the
slot, then emit the slot. Verify by xrefs from expected call sites. vtable slots
commonly differ between platforms - never assume identical indices.
