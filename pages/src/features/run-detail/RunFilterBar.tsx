  ...
  return (
    <Space wrap className="filter-row">
      <Input.Search
        ...
        style={{ width: 260 }}
      />
      <Select allowClear placeholder="状态" value={filters.status} onChange={(status) => patch({ status })} options={STATUSES.map((value) => ({ value, label: value }))} style={{ width: 130 }} />
      <Select allowClear placeholder="Phase" value={filters.phase} onChange={(phase) => patch({ phase })} options={PHASES.map((value) => ({ value, label: value }))} style={{ width: 170 }} />
      <Select allowClear placeholder="任务类型" value={filters.taskType} onChange={(taskType) => patch({ taskType })} options={[...new Set(graph.nodes.map((node) => node.node_type))].map((value) => ({ value, label: value }))} style={{ width: 150 }} />
      <Select
        allowClear
        placeholder="全部 Stage"
        value={filters.stageId}
        onChange={(stageId) => patch({ stageId, jobId: undefined })}
        options={[{ value: 'all', label: '全部 Stage' }, ...graph.stages.map((stage) => ({ value: stage.id, label: `${stage.stage_index} · ${stage.module_name}` }))]}
        style={{ width: 190 }}
      />
      <Select
        allowClear
        ...
        options={[{ value: 'all', label: '全部 Job' }, ...jobs.map((job) => ({ value: job.id, label: `${job.module_name} · ${job.platform}` }))]}
        style={{ width: 220 }}
      />
    </Space>
  )
