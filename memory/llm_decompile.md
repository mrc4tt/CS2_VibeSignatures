...
title: llm_decompile
type: note
permalink: cs2-vibesignatures/llm-decompile
---
...
## Issue #937: validate the first client acquisition, not the nearest server-full call

- Trigger: ConnectClient LLM_DECOMPILE returns a real current call instruction but CNetworkGameServer_GetFreeClient resolves to a later allocator (14178b Linux: 0x504780 instead of 0x4fc230).
- Root cause/constraint: target_disasm_codes and the instruction index are exported from the current IDB; reference VAs are examples, not live resolver inputs. Address/text checks establish instruction existence, not symbol semantics. In 14178b Linux, client acquisition has three attempts: the first GetFreeClient call, a second call inside an OR condition, then a final allocator immediately before the server-full rejection. Nearest-to-log selection chooses the wrong function.
- Correct practice: the ConnectClient finder probes current Hex-Rays ctree assignment/null-check pairs, finds the failure subtree containing both NETWORK_DISCONNECT_REJECT_SERVERFULL and Cannot get free client, and selects the unique outermost acquisition for the same client local. Keep invalid outer candidates so failed target validation cannot promote an inner fallback. Verify current function ownership and the unique direct call target. The finder-specific result validator checks the LLM call VA against that live anchor and shares the existing LLM validation retry budget. instruction_rules apply to current instruction text.
- Implementation: ida_preprocessor_scripts/_connect_client_anchor.py; find-CNetworkGameServerBase_ConnectClient-decompiles.py; optional llm_result_validator/result_validator in ida_analyze_util.py and ida_llm_decompile.py. Both reference disasm/procedure blocks annotate the first acquisition.
- Review boundary: an unsupported copy or conditional RHS immediately tested for the same client's null result remains an unverified outer blocker. Unrelated assignments are not blockers. Finder validation shares the common LLM integer parser (including IDA `1000h` and underscores), and missing or duplicate GetFreeClient entries produce retry feedback. Final live Linux and Windows anchor probes both succeeded after these guards.
- Full validation: local replay of the 14178b trusted-source plan from CI run 34164387880 covered exactly 2265 nodes and 3560 producer groups across 36 valid scopes. The final whole-tree audit found all 3557 YAMLs byte-identical. Large engine/windows execution hit the existing WinError 10048; remaining work used batches of at most 64 nodes with TCP TIME_WAIT draining. This is local replay evidence, not new CI provenance or a fix for port exhaustion. Final source tests reported 1303 tests, no failures, and three skipped Redis setup groups; the post-review dual-platform live anchor probes passed.
- Validation: 2026-09-08 isolated dual-platform rebuild yielded four byte-identical Git YAMLs. A real-IDB Linux fault injection returned 0x5067b3 (call sub_504780) first; semantic feedback rejected it, the second attempt used 0x505a04 -> 0x4fc230, and both outputs were byte-identical to Git. Unit fixtures cover first-vs-final selection, missing/ambiguous roots, invalid-first fail-closed behavior, reference addresses, ctree extraction, and correction retries.
- Scope: ConnectClient/GetFreeClient semantic recovery; unsupported or ambiguous decompiler shapes must fail closed. This does not prove all full-closure producers deterministic.
